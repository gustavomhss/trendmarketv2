...
def compare(old: dict, new: dict) -> list[str]:
    ...
    for name, prop in old_props.items():
        ...
        if prop.get('type') != new_props[name].get('type'):
            diffs.append(f"Tipo alterado para {name}: {prop.get('type')} -> {new_props[name].get('type')}")
    return diffs
...
def main() -> None:
    ...
    versions = sorted(
        [os.path.join(SCHEMA_REGISTRY, f) for f in os.listdir(SCHEMA_REGISTRY) if f.endswith('.json')]
    )
    if not versions:
