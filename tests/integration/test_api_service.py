...
def api_module(tmp_path: Path):
    ...
    module.MODERATION_SERVICE = module.ModerationService(
        audit_log=tmp_path / "moderation.log"
    )
...
def test_oracle_quote_endpoint(api_module):
    data = asyncio.run(
        api_module.get_oracle_quote(symbol="BRLUSD", repo=api_module.get_repository())
    )
...
def test_moderation_pause_endpoint(api_module):
    ...
    result = asyncio.run(
        api_module.moderation_pause(action, service=api_module.get_moderation())
    )
...
def test_resolve_apply_endpoint(api_module):
    ...
    response = asyncio.run(
        api_module.resolve_apply(request, resolver=api_module.get_auto_resolver())
    )
