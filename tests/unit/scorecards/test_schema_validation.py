...
def test_scorecard_report_schema_version_enforced(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    thresholds_path = tmp_path / "thresholds.json"
    metrics_path = tmp_path / "metrics.json"
    thresholds_path.write_text(
        THRESHOLDS_SRC.read_text(encoding="utf-8"), encoding="utf-8"
    )
    ...
    schema = json.loads(
        Path(s6_scorecards.SCHEMA_FILES["report"]).read_text(encoding="utf-8")
    )
...
def _write_stage_bundle(root: Path, stage: str) -> None:
    ...
    summary_payload = {
        ...
        "variants": {
            variant: {"status": info["status"], "notes": info["notes"]}
...
def test_boss_report_schema_version_enforced(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    schema = json.loads(
        Path(BASE_DIR / "schemas" / "q1_boss_report.schema.json").read_text(
            encoding="utf-8"
        )
    )
    validator = Draft7Validator(schema)
