...
ROOTS = [Path(".")]
SKIP = re.compile(r"(^|/)(\.git|\.ruff_cache|\.venv|venv|node_modules|out|dist|build)(/|$)")
RGX = re.compile(r'^(\s*)import\s+([^#\n]+?)(\s*#.*)?\s*$')
def split_items(s): return [x.strip() for x in s.split(",") if x.strip()]
