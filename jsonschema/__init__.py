...
class Draft7Validator:
    def __init__(
        self, schema: Dict[str, Any], format_checker: FormatChecker | None = None
    ) -> None:
        self.schema = schema
    ...
    def _validate_schema(
        self, schema: Dict[str, Any], instance: Any, path: List[Any]
    ) -> None:
        if "$ref" in schema:
            ref = schema["$ref"]
        ...
        if "type" in schema:
            ...
            if isinstance(expected_type, list):
                if not any(
                    self._check_type(candidate, instance) for candidate in expected_type
                ):
                ...
                for index, subschema in enumerate(items):
                    if index < len(instance):
                        self._validate_schema(
                            subschema, instance[index], path + [index]
                        )
                if schema.get("additionalItems") is False and len(instance) > len(
                    items
                ):
        ...
        if isinstance(instance, dict):
            ...
            for key in required:
                if key not in instance:
                    raise ValidationError(
                        f"Missing required property {key}", path + [key]
                    )
            ...
            if additional is False:
                ...
                if extra:
                    raise ValidationError(
                        f"Additional properties not allowed: {extra}", path
                    )
